...
SELECT FIRST_NAME,DATEDIFF(YEAR,YEAR_OF_JOINING,YEAR_OF_RELIEVING) AS "YEARS OF EXPERIENCE" FROM THARUN_EMPLOYEES ;

--27.Display employees who joined in the month of May.  

 SELECT * FROM THARUN_EMPLOYEES WHERE MONTH(YEAR_OF_JOINING) = '5';
...
--28.write a SQL query to find those employees who earn more than the average salary. Return employee ID, first name, last name. 
 ...
 SELECT ID,FIRST_NAME,LAST_NAME FROM THARUN_EMPLOYEES WHERE SALARY > (SELECT AVG(SALARY) FROM THARUN_EMPLOYEES);    

--29.Difference between primary key and Foreign key with exmple. 

 SELECT * FROM THARUN_EMPLOYEES LEFT OUTER JOIN THARUN_DEPARTMENTS ON THARUN_EMPLOYEES.DEPARTMENT_ID=THARUN_DEPARTMENTS.DEPARTMENT_ID;

--30.Repalce ‘null’ value to ‘AAA’  

SELECT ID,ISNULL(FIRST_NAME,'AAA') AS FIRST_NAME,ISNULL(LAST_NAME,'AAA') AS LAST_NAME FROM THARUN_EMPLOYEES;

--31.Create View table for ID,FIRST_NAME,LAST_NAME, DEPARTMENT_ID,DEPARTMENT_NAME. 

 CREATE VIEW THARUN_EMPLOYEES_VIEW AS SELECT THARUN_EMPLOYEES.ID,THARUN_EMPLOYEES.FIRST_NAME,THARUN_EMPLOYEES.LAST_NAME,THARUN_EMPLOYEES.DEPARTMENT_ID,THARUN_DEPARTMENTS.DEPARTMENT_NAME FROM THARUN_EMPLOYEES JOIN THARUN_DEPARTMENTS ON  THARUN_EMPLOYEES.DEPARTMENT_ID = THARUN_DEPARTMENTS.DEPARTMENT_ID;
    
 SELECT * FROM THARUN_EMPLOYEES_VIEW;

--32.Create SYNONYMS for EMPLOYEE table. 

CREATE SYNONYM THARUN_SYNONYM FOR THARUN_EMPLOYEES; 

SELECT * FROM THARUN_SYNONYM;

--33.Create a procedure for who are joined between 2 dates. 

    CREATE PROCEDURE THARUN_PROCEDURE AS SELECT * FROM THARUN_EMPLOYEES
    GO;
   EXEC THARUN_PROCEDURE;

    CREATE PROCEDURE THARUN_PROCEDURE_2 AS SELECT FIRST_NAME FROM THARUN_EMPLOYEES WHERE YEAR_OF_JOINING BETWEEN '2017-03-11' AND '2019-03-11';

    EXEC THARUN_PROCEDURE_2;
...
SELECT REPLACE(CONVERT(VARCHAR, '2020-12-30',101),'-','')+ REPLACE(CONVERT(, '00:38:54',108),':','')